...
exports.resendEmailVerificationToken = async (req, res) => {
  ...
  const tokenAlreadyUsed = await EmailVerificationToken.findOne({
    owner: userId,
  });
  if (tokenAlreadyUsed)
    return res.json({
      error: "Only after one hour you can request for another OTP",
    });
...
exports.forgetPassword = async (req, res) => {
  ...
  const alreadyExistToken = await PasswordResetToken.findOne({
    owner: user._id,
  });
  ...
  const resetPasswordUrl = `https://movie-review-app-nodejs.vercel.app/auth/reset-password?token=${token}&id=${user._id}`;
